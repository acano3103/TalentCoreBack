...
export class HumeService {
    ...
    async processHumeAnalysis(interviewId: string, video?: Express.Multer.File, historyRaw?: string, emotionsRaw?: string) {
        try{
            ...
            const position = await this.prisma.catPuestos.findFirst({ where: { idPuesto: mainInterview.idVacante } })
            if (!position) throw new NotFoundException('Puesto no encontrado');
            const requirements = await this.prisma.competenciasPuesto.findMany({ where: { idPuesto: mainInterview.idVacante } })
