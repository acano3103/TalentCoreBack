...
export class CatalogsSeedService implements OnModuleInit {
    ...
    private async seedHiringTypes() {
        const hiringTypes = [
            { idTipoContratacion: 1, Descripcion: 'TIEMPO COMPLETO', Activo: true },
            { idTipoContratacion: 2, Descripcion: 'MEDIO TIEMPO', Activo: true },
            { idTipoContratacion: 3, Descripcion: 'TEMPORAL', Activo: true },
            { idTipoContratacion: 4, Descripcion: 'POR PROYECTO', Activo: true },
        ]

        for (const hiringType of hiringTypes) {
            await this.prisma.$queryRaw`
                INSERT INTO CatTipoContratacion (idTipoContratacion, Descripcion, Activo)
                VALUES (${hiringType.idTipoContratacion}, ${hiringType.Descripcion}, ${hiringType.Activo})
                ON DUPLICATE KEY UPDATE Descripcion = VALUES(Descripcion), Activo = VALUES(Activo);
            `;
        }
    }

    // Seeds initial job types into the database.
    private async seedJobTypes() {
        const jobTypes = [
            { idTipoPuesto: 1, Descripcion: 'DIRECTIVO', Activo: true },
            { idTipoPuesto: 2, Descripcion: 'GERENCIAL', Activo: true },
            { idTipoPuesto: 3, Descripcion: 'EJECUTIVO', Activo: true },
            { idTipoPuesto: 4, Descripcion: 'ANALISTA', Activo: true },
            { idTipoPuesto: 5, Descripcion: 'TECNICO', Activo: true },
            { idTipoPuesto: 6, Descripcion: 'PASANTE', Activo: true },
            { idTipoPuesto: 7, Descripcion: 'OTRO', Activo: true },
            { idTipoPuesto: 8, Descripcion: 'SUPERVISOR', Activo: true },
        ]
