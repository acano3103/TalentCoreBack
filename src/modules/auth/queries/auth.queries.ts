...
export class AuthDataService {
    ...
    public async getStaffData(userId: number, fullName: string, idTenant: number | null): Promise<UserData> {
        // 1. Roles activos
        ...
        // 3. Matriz filtrada por tenant y módulo activo
        const tenantFilter = idTenant ? Prisma.sql`AND m.idTenant = ${idTenant}` : Prisma.empty;
    ...
    `;
        ...
        // 4. Jerarquía excluyendo módulos sin permiso de lectura o sin hijos activos
        const modulosPadres = permisosRaw.filter(m => m.idPadre === null && Boolean(m.puedeVer));
        const submodulos = permisosRaw.filter(m => m.idPadre !== null && Boolean(m.puedeVer));

        const modulosJerarquicos = modulosPadres
            .map(padre => {
            ...
            // Si el padre no tiene submódulos activos, no se envía al menú
            .filter(padre => padre.submodules.length > 0);
